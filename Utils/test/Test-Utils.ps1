...
function purelyWrap-Unit($inpGroups) { # input scenario groups
    ...
    function getObjLisFromCsv($inpRecLis,   # input record list of delimited strings
        ...
        $inpRecLis | Out-File $filename
        if ($delimiter -eq '') {
             $objLis = Get-ObjLisFromCsv $filename
        } else {
             $objLis = Get-ObjLisFromCsv $filename $delimiter
        }
        Remove-Item $filename
    ...
    $filename, $delimiter, $indent =  $inpGroups.'Scalars'.Split(';')
    ...
    [PSCustomObject]@{
          ...
          'Get-ObjLisFromCsv'     = getObjLisFromCsv     $inpGroups.'Get-ObjLisFromCsv'       $filename $delimiter
...
Test-Unit '.\ps_utils.json' '.\ps_utils_out.json' ${function:purelyWrap-Unit}